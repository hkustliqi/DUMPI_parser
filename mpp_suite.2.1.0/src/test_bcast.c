...
static void print_results (int64 nwrds, int64 reps, double w, uint64_t MY_GTHREAD, uint64_t GTHREADS)
...
void do_bcast  (TYPE *dst, TYPE *src, int64 nwrds, brand_t *br, int64 msize,
		int64 rep, int64 fix_root, uint64_t MY_GTHREAD, uint64_t GTHREADS)

{
  ...
  for (n = 1; n < (2 * max); n *= 2)
	...
	mpp_broadcast (dst, src, n, root, MY_GTHREAD, GTHREADS);
      }
      //mpp_barrier_all();
      ...
      for (i = 0; i < n; i++)
	x += ldst[i];
      cksum = mpp_accum_long (x, MY_GTHREAD, GTHREADS);
	...
	print_results (n, rep, t.accum_wall, MY_GTHREAD, GTHREADS);
      }
    }
...

{
  ...
  dst = mpp_alloc (nwrds * sizeof(uint64), MY_GTHREAD, GTHREADS); 
  src = mpp_alloc (nwrds * sizeof(uint64), MY_GTHREAD, GTHREADS); 
  ...
  // this exits on error
  do_bcast (dst, src, nwrds, &br, msize, niters, root, MY_GTHREAD, GTHREADS);
