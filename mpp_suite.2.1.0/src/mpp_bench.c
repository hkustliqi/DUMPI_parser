...
TYPE *mpp_alloc      (int64 nbytes, uint64_t MY_GTHREAD, uint64_t GTHREADS)
...
int64 mpp_accum_long (int64 val, uint64_t MY_GTHREAD, uint64_t GTHREADS)

{
  ...
  MPI_Allreduce (&source, &target, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
...
void  mpp_put        (uint64 *dst, uint64 *src, int64 nelem, int64 pe)
...
void  mpp_broadcast  (TYPE *dst, TYPE *src, int64 nelem, int64 root, uint64_t MY_GTHREAD, uint64_t GTHREADS)

{
  ...
  MPI_Bcast (dst, nelem, MPI_UINT64_T, root, MPI_COMM_WORLD); 
...
int64  all2all       (TYPE *dst, uint64 *src, int64 len, int64 nwrd, uint64_t MY_GTHREAD, uint64_t GTHREADS)

{
  ...
  for (i = 0; i < len; i += GTHREADS*nwrd)
	...
	//shmem_put (&dst[i + MY_GTHREAD*nwrd], &src[i + pe*nwrd], nwrd, pe);
        MPI_Win win;
        MPI_Win_create(&dst[i + MY_GTHREAD*nwrd], nwrd, sizeof(MPI_UINT64_T), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
        MPI_Win_fence(0, win);
        MPI_Put(&src[i + pe*nwrd], nwrd, MPI_UINT64_T, pe, 0, nwrd, MPI_UINT64_T, win);
        MPI_Win_fence(0, win);
	MPI_Win_free(&win);
#endif
      }
...
static uint64 do_cksum (uint64 *arr, int64 len, uint64_t MY_GTHREAD, uint64_t GTHREADS)

{
  ...
  for (i = 0; i < len; i++)
    cksum += arr[i];
  return mpp_accum_long (cksum, MY_GTHREAD, GTHREADS);
}

// exits on error

void  do_all2all     (TYPE *tab, uint64 *loc, brand_t *br, int64 msize,
		      int64 tsize, int64 rep, int64 print, uint64_t MY_GTHREAD, uint64_t GTHREADS)

{
  ...
  while (rep-- > 0) {
    ...
    // source checksum
    cksum = do_cksum (&loc[len], len, MY_GTHREAD, GTHREADS);
    ...
    timer_clear (&t);
    timer_start (&t);
    len = all2all (&tab[0], &loc[len], len, msize/sizeof(uint64), MY_GTHREAD, GTHREADS);
    //mpp_barrier_all();
    MPI_Barrier(MPI_COMM_WORLD);
    timer_stop (&t);
    // End Timing
    ...
    // dest checksum
    i = do_cksum (&loc[0], len, MY_GTHREAD, GTHREADS);
    if (i != cksum) {
...
void  do_warmup      (brand_t *br, uint64_t MY_GTHREAD, uint64_t GTHREADS)

{
  ...
  tab = mpp_alloc (NWRDS * sizeof(uint64), MY_GTHREAD, GTHREADS);
  ...
  do_all2all (tab, loc, br, (1uL<<12), NWRDS*sizeof(uint64), 8, 0, MY_GTHREAD, GTHREADS);
